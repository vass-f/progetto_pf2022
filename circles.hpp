...
class Person {
 private:
  ...
  Point velocity_{0., 0.};
  int infection_{
      0};  // integer number with the information of the infection status, 0
           // means infectable, 1 means infected, 2 means recovered, 3 means dead
  sf::CircleShape circle_;

 public:
  Person(Point position, Point velocity, int infection, sf::CircleShape circle)
      : position_{position},
        velocity_{velocity},
