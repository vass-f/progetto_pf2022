...
/*Importante: ogni volta in cui ho dovuto decidere una posizione, l'ho fatto in
funzione di percentuali rispetto alla larghezza e lunghezza della finestra, in
questo modo tutto dovrebbe essere al proprio posto qualsiasi sia la grandezza
della finestra,
...
void Finestra::add(double x) { data_.add(x); }

void Finestra::add(std::vector<double> vector) { data_.add(vector); }

sf::Text Finestra::create_text(std::string string, sf::Vector2f position,
                               int grandezza) {
  sf::Text text;
  text.setFont(font);
...
// In create_rectangle, val paremeter indicate the height, but it's actually a
// number between 0 and 1. If is 1, it's the maximum value, and its height is
// "h". If it tends to 0, the value is very low, and the height have to be near
// the x axis.
sf::VertexArray Finestra::create_rectangle(double val, int i) {
  ...
  r[1].position =
      sf::Vector2f(origin.x + i * delta_x, h + (1 - val) * (origin.y - h));
  r[2].position = sf::Vector2f(origin.x + (i + 1) * delta_x,
                               h + (1 - val) * (origin.y - h));
...
void Finestra::draw_rectangle() {
  ...
  while ((x_extreme.x - origin.x) / delta_x < data.size()) {
    delta_x -= 0.1;
  };
...
void Finestra::draw_bars() {
  ...
  for (int i = 1; i != n_int_y + 1; ++i) {
    double val =
        (double)(i) / (double)(n_int_y);  // It works with fraction because the
                                          // bars are equidistant
    auto posizione = h + (1 - val) * (origin.y - h);
    auto barra = create_y_axis_bar(posizione);
    window_.draw(barra);
    // Draw the information related to the bar
    window_.draw(create_text(
        std::to_string(int(max) - int((max / n_int_y) * (n_int_y - i))),
        sf::Vector2f(barra[0].position.x - (0.5) * barra[0].position.x,
                     barra[0].position.y - (0.06) * barra[0].position.y)));
  }
  ...
  for (int i = 1; i != (int)data_.get().size() + 1; ++i) {
    ...
    if (i % 5 == 0)  // Every five bars, draw the information related to the bar
      window_.draw(create_text(
          std::to_string(i),
          sf::Vector2f(barra[0].position.x - (0.01) * barra[0].position.x,
...
void Finestra::draw() {
  ...
  // Draw a small legend
  window_.draw(create_text(
      (label_y_ + " " + std::to_string(int(data_.get().back()))),
      sf::Vector2f((0.80) * display_width_, (0.05) * display_height_)));
  window_.draw(create_text(
      ("Day " + std::to_string(data_.get().size())),
      sf::Vector2f((0.80) * display_width_, (0.1) * display_height_)));
  window_.display();
}