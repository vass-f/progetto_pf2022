...
void epidemic::evolve(){      //In this function we implement the equation
    ...
    p_.S = before.S - (beta_ * before.S * before.I) / N();        //calculate the variations starting from the initial values
    p_.I = before.I + (beta_ * before.S * before.I) / N() - (gamma_ * before.I);
    p_.R = before.R + gamma_ * before.I;
    assert(round(p_.S + p_.I + p_.R) == N());     //we use "round" for a exact rounding
}
...
population epidemic::approx(){ //To convert from double to "int" we use "round" algorithm on I and R, then we calcolate 
    population p_integer{};    //S with a difference starting from the total. In this way we'll know that N will remain constant.
